...
**`flow-investigator`** subagent (dispatch name + resolution rule per `install/harness-notes.md` —
e.g. `flow:flow-investigator` on Claude Code; if the name doesn't resolve, retry the other
namespace form) with the symptom — it returns a root-cause hypothesis + confidence
