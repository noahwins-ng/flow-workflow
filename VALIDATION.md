...
| 2026-08-23 | opencode port | plugin load + root resolution | PASS | `.opencode/plugin/flow.ts` loads via one `plugin` line (file path, not dir); resolves FLOW_ROOT by walking up to `profile.template.yaml`. v1.18.21. |
| 2026-08-23 | opencode port | skill discovery | PASS | All 18 `flow-*` skills listed by native `skill` tool via config-hook `skills.paths` mutation (superpowers pattern; symlink approach not needed). |
| 2026-08-23 | opencode port | subagent dispatch | PASS | `flow-investigator` dispatched by flat name via task tool; followed its strict report format. |
| 2026-08-23 | opencode port | guardrail hook | PASS | Opt-in (`FLOW_HOOKS=1`): force-push blocked with script's own message; inert without opt-in. Shells out to same `hooks/protect-repo.sh`. |
| 2026-08-23 | opencode port | bootstrap injection | PASS | First-user-message injection; model reported suite location + index skill unprompted. |
| 2026-08-23 | opencode port | skill execution (read-and-follow) | PASS | `flow-status` ran from SKILL.md under opencode; empty-profile case skipped cleanly. |
| 2026-08-23 | opencode port | slash-commands | PASS | Skills auto-expose as slash commands: all 18 `/flow-*` present with plugin line alone (server-API verified). Two traps found: config-hook `command` mutation is ignored, and `opencode run "/x"` never expands slashes (TUI-only) — early "command works" observations were the model improvising off literal text. No command files shipped; zero-config. |
| 2026-08-23 | opencode port | full VALIDATION phases 1–7 on opencode | DEFERRED | Needs Linear sandbox + GitHub repo; Claude Code remains primary validated harness. `check-uncommitted.sh` not portable (no session-close event) — known gap. |
| 2026-08-26 | 3 (opencode) | flow-ship-issue QNT-314 | PASS | Native MCP `linear:connected` (`https://mcp.linear.app/mcp`), no `LINEAR_API_KEY` (native-only per user decision). Repurposed canceled QNT-314 (free-tier limit) → `Todo`. Ship via opencode (model `muse-spark-1.2-contributor-free`, batch `opencode run` — final summary only, no live stream). `bbf57b7` `QNT-314: chore(sandbox): opencode smoke — add opencode badge to docs (#7)` PR #7 `MERGED` 2026-08-26T17:00:32Z, `docs/INDEX.md:3` badge, `make test` 10 pass, `flow-code-reviewer` SHIP, Linear → `Done`. Left as test per user. |
